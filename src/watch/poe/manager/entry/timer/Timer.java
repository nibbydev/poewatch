...
public class Timer {
    ...
    public void computeCycleDelays(StatusElement statusElement) {
        Main.ADMIN.log("Calculating cycle delays...", Flair.STATUS);
        ...
        if (total > 0) {
            Main.ADMIN.log(String.format("Total delay span was %5d milliseconds", total), Flair.STATUS);
    ...
    private void _start(String key, TimerType type) {
        ...
        if (!type.equals(TimerType.NONE)) {
            if (delay != null) {
                ...
                Main.ADMIN.log(String.format("Delay  null found for key %8s", key), Flair.STATUS);
            }
        }
